...
/**
 * Dutch, with the same readability data English has.
 *
 * The pack is the seam between the language-free engine and the hand-compiled
 * word lists under resources/lang-data/nl. Everything it exposes is built once
 * here, because a pack is registered once and then asked the same questions for
 * every paper the site has.
 */
final readonly class DutchLanguagePack implements LanguagePack
{
    ...
    public function __construct()
    {
        ...
        $this->passive = new DutchPassiveVoiceDetector(
            $this->words,
        ...
        $this->syllables = new DutchSyllableCounter(is_array($syllables['deviations'] ?? null) ? $syllables['deviations'] : []);
        $this->flesch = new DutchFleschFormula;
    }
    ...
    {
        return DataFileLoader::load(__DIR__.'/../../../../resources/lang-data/nl/'.$name.'.php');
    }
}
