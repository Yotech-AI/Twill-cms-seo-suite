...
/**
 * German, with the same readability data English has.
 *
 * The pack is the seam between the language-free engine and the hand-compiled
 * word lists under resources/lang-data/de. Everything it exposes is built once
 * here, because a pack is registered once and then asked the same questions for
 * every paper the site has.
 */
final readonly class GermanLanguagePack implements LanguagePack
{
    ...
    public function __construct()
    {
        ...
        $this->passive = new GermanPassiveVoiceDetector(
            $this->words,
        ...
        $this->syllables = new GermanSyllableCounter(is_array($syllables['deviations'] ?? null) ? $syllables['deviations'] : []);
        $this->flesch = new GermanFleschFormula;
    }
    ...
    {
        return DataFileLoader::load(__DIR__.'/../../../../resources/lang-data/de/'.$name.'.php');
    }
}
